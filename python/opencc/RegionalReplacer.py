...
def _try_load_dll_backend():
    ...
    dll.opencc_get_multi_chars.argtypes = [ctypes.c_char_p, ctypes.c_char_p]
    dll.opencc_get_multi_chars.restype = ctypes.c_void_p
...
def save_regional_replaces(filepath: str):
    ...
    for cfg in by_config:
        ...
        for word in all_keys:
            ...
            has_multi = False
            for ch in word:
                multi_chars = _get_multi(cfg, ch)
                if multi_chars:
                    parts.append(f"【{'/'.join(multi_chars)}】")
                    has_multi = True
                else:
                    parts.append(ch)
            if has_multi:
...
def convert_regional(text: str, config: str = 's2tw_jieba') -> Tuple[str, List[Tuple[str, str]]]:
    """转换文本并收集地区用语替换记录
...
def _get_multi(cfg: str, ch: str) -> list:
    global _dll_handle
    if _dll_handle is None:
        return []
    try:
        ptr = _dll_handle.opencc_get_multi_chars(cfg.encode('utf-8'), ch.encode('utf-8'))
        if ptr:
