...
#if __has_include("multi_char_dict.inc")
#include "multi_char_dict.inc"
#endif

const char* opencc_get_multi_chars(const char* config, const char* text) {
#if __has_include("multi_char_dict.inc")
    if (config == nullptr || text == nullptr) {
        return "[]";
    }
    std::string dict_name = "STCharacters";
    auto config_it = kConfigToCharDict.find(config);
    if (config_it != kConfigToCharDict.end()) {
        dict_name = config_it->second;
    }
    auto dict_it = kMultiCharDict.find(dict_name);
    if (dict_it == kMultiCharDict.end()) {
        return "[]";
    }
    auto char_it = dict_it->second.find(text);
    if (char_it != dict_it->second.end()) {
        return char_it->second.c_str();
    }
#endif
    return "[]";
}
